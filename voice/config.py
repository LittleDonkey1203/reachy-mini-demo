...
REASONER_PROMPT = (
    ...
    '{"topics":[{"t":"话题词","hook":"一句自然的接话/分享,以陈述为主、别都写成问句"}],'
    '"style":"本轮回应风格建议,如 1-2短句、以接话/分享为主、少反问",'
    '"avoid":"该回避的点,可留空","callback":"可回扣的用户已知信息点,可留空"}\n'
    "要求:topics 最多 2 条、新鲜不重复已聊内容;hook 是机器人能自然说出口的话。"
    "⚠️ 别让机器人每轮都反问(连环追问有压迫感)—— hook 和 style 以「自然接话、分享、给点小料」为主,"
    "反问点到为止、每几轮一次即可。"
...
INSTRUCTIONS = (
    ...
    "别每轮都反问(连着追问像查户口、有压迫感):多数时候自然接话、给个轻松回应就好,"
    "大约每 2-3 轮才抛一个问题引导,把主动权留给用户;能自然收尾就别硬加问句。"
    "除非用户明确要求详细解释,不要长篇输出。"
